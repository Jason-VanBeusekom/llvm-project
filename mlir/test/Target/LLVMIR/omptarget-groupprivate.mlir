...
module attributes {omp.is_target_device = true, llvm.target_triple = "amdgcn-amd-amdhsa",
  ...
  // allocate a per-contention-group copy in the shared address space. This
  // mirrors the equivalence between 'target' and 'target teams num_teams(1)'.
...
// A groupprivate nested in a 'teams' region lowers to the same shared-memory
// copy as one placed directly in the enclosing 'target' region.
...
// The store in the teams-nested case targets the shared-memory copy.
